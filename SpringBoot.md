...
SpringCloud依赖于SpringBoot，使用SpringMVC编写Http协议接口，同时cloud也是一套完整的微服务解决框架。
### 创建SpringBoot工程
...
5) Mustache
### 集成lombok插件让代码更简洁
下载lombok插件并引入依赖后，在实体层entity类上加上@Data注解，能够自动生成Getter、Setter、toString方法
### 日志管理
...
### 打包运行发布
实际公司中可能用自动容器进行部署，这里手动打包。  
（1） cmd进入到项目文件夹  
（2） mvn clean package 执行maven的生命周期(从clean→package)    
（3） java -jar 全包名 ，即可运行    
（4） 当前终端ctrl+c即可停止  
如果报错没有主清单，pom.xml中新增  
    ...
    <build>
        <plugins>
            <plugin><!--识别启动类App-->
                ...
                <configuration>
                    <mainClass>com.Yz.App</mainClass>
                    <excludes>
                        <exclude>
